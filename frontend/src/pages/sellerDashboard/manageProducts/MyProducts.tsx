...
const MyProducts: React.FC = () => {
  ...
  const StatusPill: React.FC<{ status: 'Active' | 'Out of stock' }> = ({ status }) => {
    const isActive = status === 'Active';
    const base = 'inline-flex items-center justify-center rounded-[10px] h-10 w-[119px] text-[16px] font-semibold';
    return (
      <span className={isActive ? `${base} bg-[#2ECC71] text-white` : `${base} bg-[#FF0000] text-white`}>
        {status}
      </span>
    );
  };

  const MobileStatusPill: React.FC<{ status: 'Active' | 'Out of stock' }> = ({ status }) => {
    const isActive = status === 'Active';
    const base = 'inline-flex items-center justify-center rounded-[5px] h-[23px] w-[75px] text-[10px] font-medium';
    return (
      <span className={isActive ? `${base} bg-[#2ECC71] text-white` : `${base} bg-[#FF0000] text-white`}>
        {status}
      </span>
  ...
  // Load products from backend with graceful fallback
  React.useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchProducts({ page, pageSize })
      .then((res: { items: ProductListItem[]; total: number }) => {
        if (!isMounted) return;
        setItems(res.items);
        setTotal(res.total);
      })
      .catch(() => {
        ...
        const demo: ProductListItem[] = [
          { id: '1', title: 'Nokia 4G Mobile Phone - 64 GB', price: 45000, stock: 50, status: 'active', thumbnailUrl: prod1 },
          { id: '2', title: 'Men\'s Casual Cotton Slim Fit Shirt – Blue Long Sleeve', price: 1200, stock: 1, status: 'out_of_stock', thumbnailUrl: banner },
          { id: '3', title: 'Wireless Bluetooth Earphones with Mic – Deep Bass, Long Battery', price: 2500, stock: 100, status: 'active', thumbnailUrl: prod2 },
          { id: '4', title: 'Comfortable Casual Rubber Slippers – Non Slip, Soft Everyday Wear', price: 300, stock: 500, status: 'active', thumbnailUrl: prod1 },
          { id: '5', title: 'Apple MacBook Air M2 13"', price: 199999, stock: 12, status: 'active', thumbnailUrl: prod2 },
          { id: '6', title: 'Samsung Galaxy A34 5G – 128 GB', price: 79999, stock: 0, status: 'out_of_stock', thumbnailUrl: banner },
          { id: '7', title: 'Sony WH-CH520 Wireless Headphones', price: 17500, stock: 43, status: 'active', thumbnailUrl: prod1 },
          { id: '8', title: 'HP Pavilion 15 – Core i5 12th Gen', price: 165000, stock: 7, status: 'active', thumbnailUrl: prod2 },
          { id: '9', title: 'Anker 20W Fast Charger USB-C', price: 3200, stock: 150, status: 'active', thumbnailUrl: prod1 },
          { id: '10', title: 'Nike Revolution 6 Men\'s Running Shoes', price: 11500, stock: 3, status: 'out_of_stock', thumbnailUrl: banner },
          { id: '11', title: 'Logitech M331 Silent Plus Mouse', price: 3999, stock: 98, status: 'active', thumbnailUrl: prod2 },
          { id: '12', title: 'Xiaomi Redmi Buds 4 Lite', price: 5500, stock: 24, status: 'active', thumbnailUrl: prod1 },
        ];
        setItems(demo);
        ...
        <div ref={containerRef} className="bg-white rounded-[10px] md:rounded-[15px] py-1 border border-[#C1C1C1] overflow-hidden">
          {isMobile ? (
            ...
            <>
              ...
              {!loading && items.slice((page - 1) * pageSize, page * pageSize).map((it: ProductListItem) => {
                const statusText: 'Active' | 'Out of stock' = it.status === 'active' ? 'Active' : 'Out of stock';
                const thumb = it.thumbnailUrl || prod1;
                return (
                  <div key={it.id} className="grid grid-cols-[75px_1fr_40px_80px] items-center px-3 py-5 border-b last:border-b-0 border-[#C1C1C1]">
                    <div className="w-[67px] h-[61px] rounded-[10px] border border-[#B8B1B1] bg-[rgba(46,204,113,0.28)] flex items-center justify-center overflow-hidden">
                    ...
                    <div className="flex justify-center">
                      {isMobile ? (
                        <MobileStatusPill status={statusText} />
                      ) : (
            ...
            <>
              <div className="relative">
                ...
                {!loading && items.slice((page - 1) * pageSize, page * pageSize).map((it: ProductListItem) => {
                  ...
                  const thumb = it.thumbnailUrl || prod1;
                  return (
                    <div key={it.id} className="grid grid-cols-[160px_1fr_140px_120px_140px] items-center px-6 py-6 border-b last:border-b-0 border-[#C1C1C1]">
                      <div className="w-[123px] h-[113px] rounded-[10px] border border-[#B8B1B1] bg-[rgba(46,204,113,0.28)] flex items-center justify-center overflow-hidden">
                        <img src={thumb} alt="product" className="w-[82px] h-[82px] object-cover rounded-[10px]" />
                      </div>
                      <div className="text-[18px] md:text-[20px] font-medium text-black pr-4 pl-6">{it.title}</div>
                      <div className="text-[18px] md:text-[22px] font-medium text-black pl-6">{Intl.NumberFormat('en-PK').format(it.price)}</div>
                      <div className="text-[18px] md:text-[22px] font-medium text-black pl-6">{it.stock}</div>
                      <div className="pl-6">
                        <StatusPill status={statusText} />
                      </div>
                    </div>
